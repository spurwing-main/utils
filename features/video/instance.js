...
// Instance class
export function Instance(video, { INSTANCES, CONTAINER_CLAIMS, VIEW_FALLBACK }){
  ...
  if (hasNativeSrc(video)) {
    ...
    try { video.removeAttribute('src'); } catch(e){ logError('removeAttribute src failed', e); }
    try { video.src = ''; } catch(e){ logError('clear src failed', e); }
    try { video.load(); } catch(e){ logError('video.load() failed during neutralize', e); }
  }
  this.loaded = false;
  this.chosenSrc = null;
...
Instance.prototype._setup = function(){
  ...
  const onError = (e) => {
    if (this._errorTriedAlt) {
      emit(v, 'video:error', { trigger: 'manual', reason: 'media-error', url: (v.currentSrc || this.chosenSrc || null) });
    ...
    if (alt && alt !== this.chosenSrc){
      ...
      this._errorTriedAlt = true;
      emit(v, 'video:error', { trigger: 'manual', reason: 'no-alternate', url: (v.currentSrc || this.chosenSrc || null) });
  ...
  this._destroyFns.push(() => { v.removeEventListener('playing', onPlaying); });
};
...
Instance.prototype._bindPointer = function(target){
  ...
  const onLeave = () => {
    if (c.pause.onPointerOff){ this._requestPause('pointer-off'); this._pausedByPointerOff = true; }
  };
  const onCancel = () => {
    if (c.pause.onPointerOff){ this._requestPause('pointer-off'); this._pausedByPointerOff = true; }
  };
  const opts = (()=>{ try { return { passive: true }; } catch { return false; } })();
...
Instance.prototype._pickAlternate = function(){
  ...
  if (chosen && src && mob){ return (chosen === src) ? mob : src; }
  return null;
};
...
Instance.prototype._applySrc = function(url){
  ...
  try { v.removeAttribute(A.SRC); } catch(e){ logError('remove data-video-src failed', e); }
  try { v.removeAttribute(A.SRC_MOB); } catch(e){ logError('remove data-video-mob-src failed', e); }
};
...
Instance.prototype._ensureLoaded = function(trigger){
  ...
  if (!url){ emit(v, 'video:error', { trigger: trigger || 'manual', reason: 'missing-src', url: null }); return; }
  try {
    this._applySrc(url);
  } catch(e){
  ...
  if (!needVis){
    if (this.io) {
      try { this.io.unobserve(v); this.io.disconnect(); } catch(e){
        logError('teardown IO after load failed', e);
      }
      this.io = null;
    } else {
      // Using fallback - remove from VIEW_FALLBACK if no visibility behavior needed
      try { this._VIEW_FALLBACK.remove(this); } catch(e){ logError('VIEW_FALLBACK remove failed', e); }
    }
  }
};
...
Instance.prototype._requestPlay = function(trigger, priority=false){
  ...
  if (this._pausedByPointerOff && trigger === 'visible'){ return; }
        ...
        if (isPointerOn && !v.muted){
          ...
          try {
            ...
            if (p2 && typeof p2.then === 'function'){
              p2.then(() => {
                if (this.requestedPreload === 'auto' && !this.upgradedAutoPreload){ v.preload = 'auto'; this.upgradedAutoPreload = true; }
              }).catch(() => { /* POLICY-EXCEPTION: secondary play promise rejection ignored (fallback already muted) */ });
            }
          } catch { /* POLICY-EXCEPTION: second play attempt threw (cannot recover) */ }
        }
      });
    }
...
Instance.prototype._requestPause = function(trigger){
  const v = this.v;
  try { v.pause(); } catch { /* POLICY-EXCEPTION: pause failed (already paused or unsupported) */ }
...
Instance.prototype._onIntersect = function(entries){
  ...
  for (let i=0;i<entries.length;i++){
    const e = entries[i]; if (e.target !== v) continue;
...
Instance.prototype._checkVisibilityFallback = function(){
  ...
  if (this._fbLastVis !== isVis){
    ...
    this._fbStable = 0;
    // Act immediately on state change since we have stable getBoundingClientRect in tests
    if (this._visible !== isVis) {
      this._visible = isVis;
      if (isVis){
        if (c.load.onScroll && !this.loaded) {
          this._ensureLoaded('visible');
        }
        if (c.play.onVisible){
          if (this._pausedByHidden || this.v.paused) this._requestPlay('visible');
          this._pausedByHidden = false;
  ...
  // State is stable, no action needed
};
...
Instance.prototype.refresh = function(){
  ...
  // Reset fallback jitter guard state
  this._fbLastVis = null;
  this._fbStable = 0;
  this.cfg = this._readConfig();
  this._setup();
...
Instance.prototype.reloadSources = function(){
  if (!this.chosenSrc){ this._ensureLoaded('manual'); return; }
...
Instance.prototype.destroy = function(){
  ...
  for (let i=0;i<this._destroyFns.length;i++){
    try { this._destroyFns[i](); } catch(e){ logError('destroy fn error', e); }
  }
  ...
  if (this._containerBound && this._containerOwns){
    try { this._CONTAINER_CLAIMS.delete(this._containerBound); } catch(e){ logError('container claim release failed', e); }
  }
  log('[video] detached');
};